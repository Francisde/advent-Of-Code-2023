...
def get_all_arangements(row):
    ...
    return (generate_permutations_forward(row, count_1, count_2), row)
    #all_arangements = clean_up(all_arangements)


results = []
def response(result):
    print("{} : arrang: {}".format(result[1], result[0]))
...
def demo_multi_processing():
    ...
    for game in game_rows:
        pool.apply_async(get_all_arangements, args=(game, ), callback=response)
    pool.close()
    pool.join()
